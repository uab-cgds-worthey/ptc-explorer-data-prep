...
# Alternative function to run pipeline including optional script
run_ptce_data_pipeline_full <- function(verbose = TRUE, stop_on_error = TRUE) {
  
  # List of scripts including optional one
  scripts <- c(
    "src/init.R",
    "src/prepare_metadata.R", 
    "src/prepare_variant_data.R",
    "src/oncoplot_prepare_data.R",
    "src/oncoplot_alt_fun.R",  # Optional script included
    "src/oncoplot_gene_exp_boxplots.R",
    "src/oncoplot_main.R",
    "src/enrichment_analysis_enrichR.R",
    "src/prepare_fusion_deg_tbl.R",
  ...
  # Use the same logic but with full script list
  start_time <- Sys.time()
  
  if (verbose) {
    cat("===============================================\n")
    cat("PTC Explorer Data Preparation Pipeline (Full)\n")
    cat("===============================================\n")
    cat("Start time:", format(start_time, "%Y-%m-%d %H:%M:%S"), "\n\n")
  }
...
cat("Usage:\n")
cat("  run_ptce_data_pipeline()           # Run standard pipeline (excludes optional oncoplot_alt_fun.R)\n")
cat("  run_ptce_data_pipeline_full()      # Run full pipeline (includes all scripts)\n")
cat("\nOptions:\n")
