...
alter_fun = list(
  ...
  LB = function(x, y, w, h)
    grid.rect(x, y-0.4*h, w*0.9, h*0.1, gp = gpar(fill = "lightgreen", col = NA)),
  # dots
  ...
  Normal = function(x, y, w, h) 
    grid.rect(x, y, w*0.9, h*0.9, gp = gpar(fill = NA, lwd = 2, col = "blue")),
...
meta_col_top <- list(
  Subtypes = c("FA" = "#E31A1C", "FTC" = "#FDBF6F",
...
meta_col_bottom = list(
  ...
  Age_at_diagnosis = circlize::colorRamp2(c(
    ...
    max(ptc_meta_summary$Age_at_diagnosis, na.rm = TRUE)), c("white", "darkmagenta")),
  Batch = c("1" ="cyan", "2"="magenta", "3"="darkgreen"),
  Pathology_Subtype = c("NA" = "black", "PTC conventional variant" = "#9E9AC8",  "PTC conventional variant and papillary microcarcinoma" = "#807DBA", "PTC diffuse sclerosing variant" = "#6A51A3", "PTC follicular variant" = "#3F007D", "PTC oncocytic variant" =  "#54278F", "FTC minimally invasive" = "#FD8D3C"),  
  Nodule1_FNA_Result = c("NA" = "black", "Undertermined" = "#EFD5DF", "Benign" = "#DFACBF",
                         "Follicular_lesion_or_neoplasm" = "#CF829F",
                         "Malignant" =  "#BF597F", "Suspicious_for_malignancy" =  "#B03060"),    
  Autoimmune_Thyroiditis = c("NA"="black", "Yes" = "red", "No" = "mistyrose"),
  History_of_Radiation_to_Neck = c("NA" = "black", "Yes" = "red", "No" = "mistyrose"),
  History_of_Previous_Cancer = c("NA" = "black", "Yes" = "red", "No" = "mistyrose"),
  Received_RAI = c("NA" = "black", "Yes" = "red", "No" = "mistyrose"),
  Additional_Surgery = c("NA" = "black", "Yes" = "red", "No" = "mistyrose"),
  Race = c("NA"="black", "White_Caucasian" = "moccasin",
           "Black_or_African_American" = "saddlebrown", "Asian"= "rosybrown"),
  TI_RADS_Score = c("NA" = "black", "2" = "lightpink", "3" ="hotpink", "4"="deeppink", "5"="darkmagenta"),
  Type_of_Thyroid_Surgery = c("NA" = "black",
  ...
  Primary_Tumor = c("NA" = "black", "0" = "white",
                    "T1b" = "mediumturquoise",
                    "T2" = "darkturquoise",
                    "T3" = "darkcyan",
                    "T4a" = "darkslategray"), 
  Lymph_Nodes = c("NA" = "black", "0" = "white",
                  "N0" = "#EDE0C2", "N1a" = "#DBC285" , "N1b" = "#C9A448", "NX" = "#B8860B"),   Distant_Metastases = c("NA" = "black",
                         "0" = "white",
                         "M0" = "#D29494",
                         "M1" = "#BB5F5F", 
                         "MX" = "#A52A2A"), 
  Largest_Nodule_Dimension = circlize::colorRamp2(c(
    ...
    max(ptc_meta_summary$Largest_Nodule_Dimension, na.rm = TRUE)), c("white", "#D9691E")),
  ATA_Pediatric_Risk_Level = c("NA" = "black",
                               "High risk" = "#FF0000",
...
##### Top annotation ####
top_df <- as.data.frame(ptc_meta_summary[,"Subtypes"])
...
bottom_ann <- HeatmapAnnotation(df = bottom_df, 
                                col = meta_col_bottom,
                                annotation_legend_param = list(
                                  direction = "horizontal",
                                  title_gp = gpar(fontsize = font_label_ann+1,
                                                  fontface = "bold"),
                                  labels_gp = gpar(fontsize = font_label_ann),
...
png(file_name,
    width = 10.5, height = 24, units = "in", res = 400
    #pointsize = 12, #bg = "white"
    )


######### Create Oncoplot ############
ht <- oncoPrint(mat=ptc_df_onco_ready,
                alter_fun = alter_fun,
                col = c(P = "#CD1076", LP = "#E9967A", VUS = "#808000", LB = "lightgreen"),
                show_column_names = TRUE,
                show_row_names = TRUE,
                #  height = unit(10, "in"),
                heatmap_legend_param =  list(
                  title = "Alterations",
                  at = c("P", "LP", "VUS", "LB", "INS","SNV", "SUB", "DEL", "Tumor", "Normal"),
                  # , "LB" These match the names of the mutations defined in alter_fun
                  labels = c("Pathogenic", "Likely Pathogenic",
                             "VUS", "Likely Benign",
                             "Insertion","SNV", "Substitution", "Deletion", "Tumor", "Normal"
                  ), # , "Likely Benign"
                  title_gp = gpar(fontsize = font_label_ann+1,
                                  fontface = "bold"),
                ...
                column_split = factor(ptc_meta_summary$Subtypes, c("THY", "PTC", "PTCplusTHY","FTC","NIFTP","FA")),
                column_title_gp = gpar(fontsize = font_label_ann,
                                       fontface = "bold"),
                column_names_gp = gpar(fontsize = font_label_ann+1,
                                       fontface = "bold"),
                row_names_gp = gpar(fontsize = font_label_ann+1,
                                    fontface = "bold"),
                right_annotation = c(
                  rowAnnotation(
...
saveRDS(ht, paste0(output_dir,"oncoplot_boxplot_",
                   Sys.Date(),
                   ".rds"))
